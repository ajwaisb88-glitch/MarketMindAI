...
class BinanceOrderBookFeed:
    ...
    def observe(self, symbol: str, window_sec: float = 1.0) -> tuple[OrderBookSnapshot, list[FlowEvent]]:
        ...
        # Adds / cancels inferred from the depth diff (d0 -> d1)
        for side, pk, sk in (("bid", "bid_prices", "bid_sizes"), ("ask", "ask_prices", "ask_sizes")):
            ...
            for price, size_b in b.items():
                size_a = a.get(price, 0.0)
                if size_b > size_a * 1.05:      # size grew → add
                    events.append(FlowEvent(time.time(), side, "add", size_b - size_a,
                                            abs(price - mid) / tick if tick else 0.0))
            for price, size_a in a.items():
                size_b = b.get(price, 0.0)
                if size_b < size_a * 0.95:      # size shrank/pulled → cancel
                    events.append(FlowEvent(time.time(), side, "cancel", size_a - size_b,
                                            abs(price - mid) / tick if tick else 0.0))
        return snap, events
